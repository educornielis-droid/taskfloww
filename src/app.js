  ...
  try {
    const { rows } = await pool.query(`
      SELECT
        ...
        COALESCE(SUM(duration_seconds) / 3600.0, 0) AS hours
      FROM time_logs
      WHERE
        profile_id = $1
    ...
    // Construir array Dom-Sáb con 0 donde no hay datos
    const days = ['Dom','Lun','Mar','Mié','Jue','Vie','Sáb'];
    const result = days.map((d, i) => {
      const found = rows.find(r => parseInt(r.dow) === i);
      return {
        day: d,
        hours: found ? parseFloat(parseFloat(found.hours).toFixed(1)) : 0
      };
    });
    res.json(result);
  } catch(e) { res.status(500).json({ error: e.message }); }
