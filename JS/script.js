...
async function doLogin(e) {
  ...
  try {
    ...
    sessionStorage.setItem('tf_user', JSON.stringify(response.user));
    sessionStorage.setItem('tf_access_token', response.access_token);
...
function initKanbanFilters() {
  ...
  const saved = sessionStorage.getItem('kb_proj_filter');
  const savedMine = sessionStorage.getItem('kb_mine_filter');
  if(saved) sel.value = saved;
  if(savedMine === 'true') {
    document.getElementById('kb-filter-mine')?.classList.add('kb-filter-active');
  }
}

function toggleMineFilter() {
  const btn = document.getElementById('kb-filter-mine');
  const isActive = btn.classList.toggle('kb-filter-active');
  sessionStorage.setItem('kb_mine_filter', isActive);
  applyKanbanFilter();
}

async function applyKanbanFilter() {
  const projId = document.getElementById('kb-proj-filter')?.value || '';
  const mineOnly = document.getElementById('kb-filter-mine')?.classList.contains('kb-filter-active');
  const uid = getEffectiveUserId();

  // Guardar en sessionStorage
  sessionStorage.setItem('kb_proj_filter', projId);

  try {
    let url = '/api/tareas';
    const params = [];
    if(projId) params.push('proyecto_id=' + projId);
    if(mineOnly && uid) params.push('assigned_to=' + uid);
...
(async function init(){
  ...
  // ⭐ INTENTAR RESTAURAR SESIÓN VÁLIDA DESDE SESSIONSTORAGE
  const saved = sessionStorage.getItem('tf_user');
  const accessToken = sessionStorage.getItem('tf_access_token');
